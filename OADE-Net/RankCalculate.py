...
def rankClaculate(datasetType, modelType, Fold, imgType):
    ...
    for pickleFilePath in pickleFilePaths:
        ...
        plt.plot(range(1, rankWide+1), rankResult, 'ro', label=labelType)
        plt.plot(range(1, rankWide+1), rankResult, 'r')
        plt.xlabel('Rank')
        plt.ylabel('TPR')
        plt.xticks(range(1,rankWide+1))
        plt.title('RankScore_' + foldType + '_' + labelType + '_' + modelType)
        plt.legend(loc='lower right')
        plt.savefig(folderPath + '/RankScore_' + datasetType + '_' + foldType + '_' + labelType + '_' +
                    modelType + '.png')

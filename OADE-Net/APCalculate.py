...
def apCalculate(datasetType, modelType, Fold, imgType):
    ...
    for pickleFilePath in pickleFilePaths:
        ...
        for i, (key, value) in enumerate(ReIDdict.items()):
            ...
            plt.title(
                'PRCurve_' + foldType + '_' + labelType + '_' + str(key) + ' AP : ' + str(AP))
            plt.fill_between(apRecall[0:len(apRecall)], apPrecision[0:len(apPrecision)], alpha=0.2)

            plt.savefig(
                graphPath + '/PRCurve_' + foldType + '_' + labelType + '_' + modelType + '_' + str(key) + '_' + str(
                    AP) + '.png')
            np.save(numpyPath + '/apRecall_' + foldType + '_' + labelType + '_' + modelType + '_' + str(key),
                    np.array(apRecall))
            np.save(numpyPath + '/apPrecision_' + foldType + '_' + labelType + '_' + modelType + '_' + str(key),
